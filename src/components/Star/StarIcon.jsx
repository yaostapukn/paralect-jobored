...
function StarIcon({ vacancyProp }) {
  const [isChecked, setIsChecked] = useState(false)
  const fillStar = isChecked ? { fill: '#5E96FC', stroke: '#5E96FC' } : {}
  useEffect(() => {
    const data = localStorage.getItem('fav')
    if (data) {
      const fav = JSON.parse(data)

      if (fav.some((item) => item.id === vacancyProp.id)) {
        setIsChecked(true)
      }
    }
  }, [vacancyProp.id])

  const handleChangeFillFav = () => {
    setIsChecked(!isChecked)
    const data = localStorage.getItem('fav')
    let vacancyFav = []
    if (data) {
      vacancyFav = JSON.parse(data)
    }
    if (isChecked) {
      vacancyFav = vacancyFav.filter((item) => item.id !== vacancyProp.id)
    } else {
      vacancyFav.push(vacancyProp)
    }
    localStorage.setItem('fav', JSON.stringify(vacancyFav))
  }
